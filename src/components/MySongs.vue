    ...
    setup(props) {
      ...
      const getSavedSongs = async () => {
        const response = await axios.get(
          "http://localhost:8090/api/saved-songs", getAuthHeader()
        );
      ...
      const saveSong = async (song) => {
        await axios.post("http://localhost:8090/api/song", song, getAuthHeader());
      };
  
      // function to remove a song
      const removeSong = async (songId) => {
        await axios.delete(`http://localhost:8090/api/song/${songId}`, getAuthHeader());
        await getSavedSongs();
      };
